...
namespace KerbalAlarmClock
{
    public partial class KerbalAlarmClock
    {
        ...
        private void DrawToolTip()
        {
            ...
            if (strToolTipText != "" && (Time.unscaledTime <= fltTooltipTime))
            {
                ...
                if (!blnToolTipDisplayed || (strToolTipText != strLastTooltipText))
                {
                    ...
                    rectToolTipPosition.height = KACResources.styleTooltipStyle.CalcHeight(contTooltip, rectToolTipPosition.width); // here's the result
        ...
        private Int32 intAddPaneWindowWidth = 348;
        private Int32 AddWindowHeight;
        private Int32 EarthWindowHeight = 216;
        private Int32 QuickWindowHeight = 28;
        private Int32 intMainWindowHeightLatch = 0;
        private Int32 intAlarmListHeightLatch = 0;
        private Int32 intAlarmListCapHeightLatch = 0;
        private Int32 intAlarmListHeightPad = 6;

        private Int32 SumAlarmLineHeightExtra(List<KACAlarm> listToSum, Int32 maxItems = Int32.MaxValue)
        {
            Int32 intReturn = 0;
            Int32 intCount = 0;
            foreach (KACAlarm tmpAlarm in listToSum)
            {
                if (intCount >= maxItems)
                    break;
                intReturn += tmpAlarm.AlarmLineHeightExtra;
                intCount++;
            }
            return intReturn;
        }
        ...
        internal void DrawWindows()
        {
...
#if DEBUG
            if (_ShowDebugPane)
                _WindowDebugRect = ClickThruBlocker.GUILayoutWindow(_WindowDebugID, _WindowDebugRect, FillDebugWindow, Localizer.Format("#LOC_KAC_27"));
#endif
            ...
            {
                MainWindowPos.height = intMainWindowMinHeight + 1;
            }
            else
            {
                ...
                if (alarmsDisplayed.Count > 1)
                {
                    ...
                    {
                        MainWindowPos.height = intMainWindowBaseHeight + ((alarmsDisplayed.Count - 1) * intMainWindowAlarmListItemHeight) + SumAlarmLineHeightExtra(alarmsDisplayed);
                        //Debug.Log("[KerbalAlarmClock] a: " + size.ToString() + ", intMainWindowAlarmListItemHeight: " + intMainWindowAlarmListItemHeight +
                    ...
                    {
                        //this is scrolling
                        MainWindowPos.height = (intMainWindowBaseHeight - 0) + ((settings.AlarmListMaxAlarmsInt - 1) * intMainWindowAlarmListItemHeight) +
                            SumAlarmLineHeightExtra(alarmsDisplayed, settings.AlarmListMaxAlarmsInt) + intMainWindowAlarmListScrollPad;
                    }
                }
            ...
            //Dynamic height: latch wins (content+5.5, scale-proof); model = first-frame default
            MainWindowPos.height = intMainWindowHeightLatch > 0 ? intMainWindowHeightLatch : MainWindowPos.height;
            ...
            //Now show the window
            WindowPosByActiveScene = ClickThruBlocker.GUILayoutWindow(_WindowMainID, MainWindowPos, FillWindow, Localizer.Format("#LOC_KAC_28") + " " + settings.Version, KACResources.styleWindow);
            ...
            if (winConfirmAlarmDelete.Visible)
            {
                ...
                ClickThruBlocker.GUILayoutWindow(winConfirmAlarmDelete.windowID,
                    ...
                    winConfirmAlarmDelete.FillWindow, Localizer.Format("#LOC_KAC_29"), KACResources.styleWindow);
            }
            //Do we have anything to show in the right pane
            if (_ShowSettings)
            {
                _WindowSettingsRect = ClickThruBlocker.GUILayoutWindow(_WindowSettingsID, GetChildWindowRect(WindowPosByActiveScene, WindowPosByActiveScene.y, intSettingsPaneWindowWidth,
                    intSettingsHeightLatch > 0 ? intSettingsHeightLatch : intSettingsHeight, ref _ShowShowSettingsOnLeft, settings.WindowChildPosBelow), FillSettingsWindow, Localizer.Format("#LOC_KAC_30"), KACResources.styleWindow);
            }
            else if (_ShowAddPane)
            {
                ...
                _WindowAddRect = ClickThruBlocker.GUILayoutWindow(_WindowAddID, GetChildWindowRect(WindowPosByActiveScene, WindowPosByActiveScene.y, intAddPaneWindowWidth,
                    AddWindowHeight, ref _ShowAddPaneOnLeft, settings.WindowChildPosBelow), FillAddWindow, Localizer.Format("#LOC_KAC_31"), KACResources.styleWindow);
            ...
            else if (_ShowEarthAlarm)
            {
                ...
                _WindowEarthAlarmRect = ClickThruBlocker.GUILayoutWindow(_WindowEarthAlarmID, GetChildWindowRect(WindowPosByActiveScene, _WindowEarthTop, intAddPaneWindowWidth,
                    EarthWindowHeight, ref _ShowEarthAlarmOnLeft, settings.WindowChildPosBelow), FillEarthAlarmWindow, Localizer.Format("#LOC_KAC_32"), KACResources.styleWindow);
                if (_ShowAddMessages)
                {
                    _WindowAddMessagesRect = ClickThruBlocker.GUILayoutWindow(_WindowAddMessagesID, new Rect(_WindowEarthAlarmRect.x + _WindowEarthAlarmRect.width,
                        _WindowEarthAlarmRect.y, 200, EarthWindowHeight), FillAddMessagesWindow, "");
                }
            }
            else if (_ShowEditPane)
            {
                _WindowEditRect = ClickThruBlocker.GUILayoutWindow(_WindowEditID, GetChildWindowRect(WindowPosByActiveScene, WindowPosByActiveScene.y, intPaneWindowWidth,
                    intAlarmEditHeight, ref _ShowEditPaneOnLeft, settings.WindowChildPosBelow), FillEditWindow, Localizer.Format("#LOC_KAC_33"), KACResources.styleWindow);
            }
            else if (_ShowQuickAdd)
            {
                _WindowQuickAddRect = ClickThruBlocker.GUILayoutWindow(_WindowQuickAddID, GetChildWindowRect(WindowPosByActiveScene, WindowPosByActiveScene.y, 300,
                    QuickWindowHeight, ref _ShowQuickAddOnLeft, settings.WindowChildPosBelow), FillQuickWindow, Localizer.Format("#LOC_KAC_34"), KACResources.styleWindow);
            }

            if (_ShowBackupFailedMessage)
            {
                _WindowBackupFailedRect = ClickThruBlocker.GUILayoutWindow(_WindowBackupFailedID, _WindowBackupFailedRect, FillBackupFailedWindow, Localizer.Format("#LOC_KAC_35"), KACResources.styleWindow);
                if (DateTime.Now.Subtract(_ShowBackupFailedMessageAt).TotalSeconds > _ShowBackupFailedMessageForSecs)
        ...
        internal void FillWindow(Int32 intWindowID)
        {
            ...
            GUILayout.Label(Localizer.Format("#LOC_KAC_36"), KACResources.styleHeadingNoWrap, GUILayout.Width(65));
            GUILayout.FlexibleSpace();

            if (TimeWarp.fetch != null)
            {
                KACAlarm aNext = null;
                Double dblUT = Planetarium.GetUniversalTime();
                foreach (KACAlarm a in alarms)
                {
                    if (!a.Actioned && a.AlarmTimeUT > dblUT && (aNext == null || a.AlarmTimeUT < aNext.AlarmTimeUT))
                        aNext = a;
                }
                GUI.enabled = (TimeWarp.fetch.current_rate_index == 0 && aNext != null);
                if (GUILayout.Button(new GUIContent(KACResources.btnWarpNext, Localizer.Format("#LOC_KAC_592")), KACResources.styleHeader22Button))
            ...
            {
                if (DrawToggle(ref blnFilterToVessel, new GUIContent(KACResources.btnRocket, Localizer.Format("#LOC_KAC_37")), KACResources.styleHeader22Button))
                {
            ...
            {
                GUIContent XferIcon = new GUIContent(KACResources.iconAp, Localizer.Format("#LOC_KAC_43"));
                GUILayout.Label(XferIcon, KACResources.styleFlagIcon);
            }
            if (settings.AlarmXferRecalc)
            {
                GUIContent XferIcon = new GUIContent(KACResources.iconXFer, Localizer.Format("#LOC_KAC_44"));
                GUILayout.Label(XferIcon, KACResources.styleFlagIcon);
            }
            ...
            if (settings.AlarmAddSOIAuto || settings.AlarmSOIRecalc)
            {
                ...
                if (settings.AlarmAddSOIAuto)
                {
                    ...
                    {
                        SOITooltip += Localizer.Format("#LOC_KAC_47");
                    }
                    if (settings.AlarmAddSOIAuto_ExcludeDebris)
                    {
                        if (!SOITooltip.Contains(Localizer.Format("#LOC_KAC_48")))
                            SOITooltip += Localizer.Format("#LOC_KAC_49");
                        else
                            SOITooltip += Localizer.Format("#LOC_KAC_50");
                    }
            ...
            GUIContent contMaxMin = new GUIContent(KACResources.btnChevronUp, Localizer.Format("#LOC_KAC_53"));
            if (WindowMinimizedByActiveScene)
            {
                contMaxMin.image = KACResources.btnChevronDown;
                contMaxMin.tooltip = Localizer.Format("#LOC_KAC_54");
            }
            //Draw the button
            ...
            GUIContent contSettings = new GUIContent(KACResources.GetSettingsButtonIcon(settings.VersionAttentionFlag), Localizer.Format("#LOC_KAC_55"));
            if (settings.VersionAvailable) contSettings.tooltip = Localizer.Format("#LOC_KAC_56");
            ...
            if (DrawToggle(ref _ShowAddPane, new GUIContent(KACResources.btnAdd, Localizer.Format("#LOC_KAC_57")), KACResources.styleSmallButton) && _ShowAddPane)
            {
                //reset the add stuff
                NewAddAlarm();
            ...
            GUILayout.Space(-5);
            if (DrawToggle(ref _ShowQuickAdd, new GUIContent("+", Localizer.Format("#LOC_KAC_58")), KACResources.styleQAButton) && _ShowQuickAdd)
            {
                ResetPanes();
                _ShowQuickAdd = true;
                SetupQuickList();
            ...
            WindowLayout_AlarmList(WindowMinimizedByActiveScene);
            ...
            if (GUILayout.Button(new GUIContent(Localizer.Format("#LOC_KAC_59"), Localizer.Format("#LOC_KAC_60")), KACResources.styleAddSectionHeadingNoWrap))
            ...
            {
                GUILayout.Space(10);
                if (GUILayout.Button(new GUIContent(KACResources.btnCalendar, Localizer.Format("#LOC_KAC_61")), KACResources.styleSmallButton))
                {
            ...
            GUIContent contCurrentTime = new GUIContent(KACWorkerGameState.CurrentTime.ToStringStandard(MainClockFormat), Localizer.Format("#LOC_KAC_62"));
            GUILayout.FlexibleSpace();
            if (GUILayout.Button(contCurrentTime, KACResources.styleContentNoWrap))
            {
            ...
            {
                GUILayout.BeginHorizontal();
                if (GUILayout.Button(new GUIContent(Localizer.Format("#LOC_KAC_63"), Localizer.Format("#LOC_KAC_64")), KACResources.styleHeadingEarth))
                {
                    settings.ShowEarthTime = !settings.ShowEarthTime;
                }
                GUILayout.FlexibleSpace();
            ...
            //Dynamic height: latch measured content bottom (y = content bottom)
            Rect rectMainWindowContentEnd = GUILayoutUtility.GetRect(0, 1);
            if (Event.current.type == EventType.Repaint)
                intMainWindowHeightLatch = (Int32)(rectMainWindowContentEnd.y + 5.5f);
            GUILayout.EndVertical();
            SetTooltipText();
        ...
        private KACAlarm FindMinimizedAlarm()
        {
            ...
            if (alarms != null)
            {
                ...
                else
                {
                    foreach (KACAlarm tmpAlarm in alarms)
                    {
                        if (nextAlarm == null || tmpAlarm.AlarmTime.UT < nextAlarm.AlarmTime.UT)
                            nextAlarm = tmpAlarm;
                    }
                }
            }
        ...
        private void DrawAlarmListEmptyRow(String strText)
        {
            GUILayout.Space(4);
            GUILayout.Label(strText, KACResources.styleLabelStretch, GUILayout.Height(23));
        }
        ...
        private void WindowLayout_AlarmList(Boolean blnMinimizedList)
        {
            if (blnMinimizedList)
                scrollPosition = Vector2.zero;

            //Dynamic list height: exact for all alarms, capped at AlarmListMaxAlarms rows then scrollbar
            Boolean blnScrollCapped = !blnMinimizedList && alarmsDisplayed != null && alarmsDisplayed.Count > settings.AlarmListMaxAlarmsInt && intAlarmListCapHeightLatch > 0;
            int intListHeight = intAlarmListHeightLatch;
            if (blnScrollCapped)
                intListHeight = intAlarmListCapHeightLatch;
            if (intListHeight > 0)
                intListHeight += intAlarmListHeightPad;
            if (intListHeight < intMainWindowAlarmListItemHeight)
                intListHeight = intMainWindowAlarmListItemHeight;
            scrollPosition = GUILayout.BeginScrollView(scrollPosition, false, false, KACResources.styleScrollH, blnScrollCapped ? KACResources.styleScrollV : KACResources.styleScrollVThin, KACResources.styleScrollViewBox, GUILayout.Height(intListHeight));

            float fltListContentTop = GUILayoutUtility.GetRect(0, 1).y;
            ...
            String strCurrentVesselID = null;
            if (blnFilterToVessel && KACWorkerGameState.CurrentVessel != null)
                strCurrentVesselID = KACWorkerGameState.CurrentVessel.id.ToString();
            ...
            for (int i = 0, iAlarmCount = alarms.Count; i < iAlarmCount; i++)
            {
                ...
                {
                    if (strCurrentVesselID != null && alarms[i].VesselID == strCurrentVesselID)
                    {
                        alarmsDisplayed.Add(alarms[i]);
                    }
                }
            ...
            if (blnMinimizedList)
            {
                KACAlarm nextAlarm = FindMinimizedAlarm();
                if (nextAlarm == null)
                {
                    DrawAlarmListEmptyRow(Localizer.Format("#LOC_KAC_65"));
                }
                else
                {
                    GUILayout.Space(4);
            ...
            {
                DrawAlarmListEmptyRow(Localizer.Format("#LOC_KAC_66"));
            }
            else if (blnFilterToVessel && alarmsDisplayed.Count < 1)
            {
                if (KACWorkerGameState.CurrentVessel != null)
                    DrawAlarmListEmptyRow(Localizer.Format("#LOC_KAC_67"));
                else
                    DrawAlarmListEmptyRow(Localizer.Format("#LOC_KAC_68"));
            }
            else
            {
                GUILayout.Space(4);
                ...
                for (int i = 0, iAlarmsCount = alarmsDisplayed.Count; i < iAlarmsCount; i++)
                {
                    ...
                    if (i == settings.AlarmListMaxAlarmsInt - 1)
                    {
                        Rect rectListCapEnd = GUILayoutUtility.GetRect(0, 1);
                        if (Event.current.type == EventType.Repaint)
                            intAlarmListCapHeightLatch = (Int32)Mathf.Ceil(rectListCapEnd.y - fltListContentTop);
                    }
                }
                ...
                if (alarmList_AlarmsToRemove.Count > 0)
                {
                    ...
                    //[infradmin] The commented code below causes a bug: manual deletion of any alarm unpauses the game unconditionally
                    //is the game paused, yet we deleted any active pause alarms??
                    //Boolean blnPauseAlarmOpen = false;
                    //foreach (KACAlarm tmpAlarm in alarms)
                    //{
                    //    if (tmpAlarm.AlarmWindowID != 0 && tmpAlarm.PauseGame == true)
                    //    {
                    //        blnPauseAlarmOpen = true;
                    //        break;
                    //    }
                    //}
                    //if (!blnPauseAlarmOpen)
                    //{
                    //    if (FlightDriver.Pause)
            ...
            Rect rectListContentEnd = GUILayoutUtility.GetRect(0, 1);
            if (Event.current.type == EventType.Repaint)
                intAlarmListHeightLatch = (Int32)Mathf.Ceil(rectListContentEnd.y - fltListContentTop);
        ...
        private Boolean DrawAlarmLine(KACAlarm tmpAlarm)
        {
            ...
            using (new GUILayout.HorizontalScope(GUILayout.Height(23)))
            {
                ...
                Int32 intMaxwidth = (Int32)WindowPosByActiveScene.width - 84;
                if (_ShowEditPane && (alarmEdit == tmpAlarm)) intMaxwidth = (Int32)WindowPosByActiveScene.width - 105;
                tmpAlarm.AlarmLineHeight = Convert.ToInt32(styleLabel.CalcHeight(contAlarmLabel, intMaxwidth));

                if (GUILayout.Button(contAlarmLabel, styleLabel, GUILayout.MaxWidth((Int32)WindowPosByActiveScene.width - 84), GUILayout.ExpandWidth(true)))
                {
                    if (!_ShowSettings)
                    {
                ...
                if ((alarmEdit == tmpAlarm) && _ShowEditPane)
                {
                    GUILayout.Label(new GUIContent(KACResources.iconEdit, Localizer.Format("#LOC_KAC_69")), KACResources.styleLabelWarp);
                }
                if (tmpAlarm.PauseGame)
                {
                    GUILayout.Label(new GUIContent(KACResources.GetPauseListIcon(tmpAlarm.WarpInfluence), Localizer.Format("#LOC_KAC_70")), KACResources.styleLabelWarp);
                }
                else if (tmpAlarm.HaltWarp)
                {
                    ...
                    if (tmpAlarm.Actions.Message != AlarmActions.MessageEnum.No)
                        GUILayout.Label(new GUIContent(KACResources.GetWarpListIcon(tmpAlarm.WarpInfluence), Localizer.Format("#LOC_KAC_71")), KACResources.styleLabelWarp);
                    else
                        GUILayout.Label(new GUIContent(KACResources.GetWarpListIcon(tmpAlarm.WarpInfluence), Localizer.Format("#LOC_KAC_72")), KACResources.styleLabelWarp);
                }
                else
                {
                ...
                if (GUILayout.Button(new GUIContent(KACResources.btnRedCross, Localizer.Format("#LOC_KAC_73")), KACResources.styleRedCrossButton, GUILayout.MaxWidth(20), GUILayout.MaxHeight(20)))
                    blnReturn = true;
                GUILayout.Space(3);
            }
            return blnReturn;
        ...
        {
            //Two Columns
            GUILayout.Label(Localizer.Format("#LOC_KAC_74"), KACResources.styleAddSectionHeadingNoWrap);
            ...
            GUILayout.Label(Localizer.Format("#LOC_KAC_75"), KACResources.styleAddHeadingNoWrap);
            GUILayout.Label(Localizer.Format("#LOC_KAC_76"), KACResources.styleAddHeadingNoWrap);
            GUILayout.EndVertical();
            GUILayout.BeginVertical(GUILayout.Width(260), GUILayout.MaxWidth(260));
            strName = TruncateInput(GUILayout.TextField(strName, KACResources.styleAddField).Replace("|", ""), 120);
            GUIStyle styleAddWrap = KACResources.styleAddFieldWrap;
            strMessage = TruncateInput(GUILayout.TextArea(strMessage, styleAddWrap).Replace("|", ""), 150);
            GUILayout.EndVertical();
            GUILayout.EndHorizontal();

            //Full width one under the two columns for the kill time warp
            DrawAlarmActionChoice4(ref Actions, "On Alarm:", 90);
            ...
            if (TypeOfAlarm != KACAlarm.AlarmTypeEnum.Raw && TypeOfAlarm != KACAlarm.AlarmTypeEnum.EarthTime && TypeOfAlarm != KACAlarm.AlarmTypeEnum.Crew && TypeOfAlarm != KACAlarm.AlarmTypeEnum.ScienceLab)
                DrawTimeEntry(ref Margin, KACTimeStringArray.TimeEntryPrecisionEnum.Hours, Localizer.Format("#LOC_KAC_77"), 90);
            GUILayout.EndVertical();
        }
        ...
        //    //Full width one under the two columns for the kill time warp
        //    DrawAlarmActionChoice3(ref Action, "Action:", 70 ,38);
        ...
        private void WindowLayout_CommonFields3(ref String strName, ref Boolean blnAttachVessel, ref AlarmActions Actions, ref KACTimeStringArray Margin, KACAlarm.AlarmTypeEnum TypeOfAlarm, Int32 WindowHeight)
        {
            ...
            GUILayout.Label(strTitle, KACResources.styleAddSectionHeadingNoWrap);
            GUILayout.BeginVertical(KACResources.styleAddFieldAreas, GUILayout.Height(WindowHeight));
            GUILayout.Space(4);
            ...
            {
                GUILayout.BeginHorizontal();
                GUILayout.Label(Localizer.Format("#LOC_KAC_80"), KACResources.styleAddHeading);
                String strVesselName = Localizer.Format("#LOC_KAC_81");
                if (KACWorkerGameState.CurrentVessel != null) strVesselName = KACWorkerGameState.CurrentVessel.vesselName;
                GUILayout.Label(strVesselName, KACResources.styleLabelWarning);
                GUILayout.EndHorizontal();
            }
            ...
            GUILayout.BeginHorizontal();
            GUILayout.Label(Localizer.Format("#LOC_KAC_82"), KACResources.styleAddHeadingNoWrap, GUILayout.Width(41));
            strName = TruncateInput(GUILayout.TextField(strName, KACResources.styleAddField, GUILayout.MinWidth(0)).Replace("|", ""), 120);

            GUIContent guiBtnMessages = new GUIContent(KACResources.btnChevRight, Localizer.Format("#LOC_KAC_83"));
            if (_ShowAddMessages) guiBtnMessages = new GUIContent(KACResources.btnChevLeft, Localizer.Format("#LOC_KAC_84"));
            if (GUILayout.Button(guiBtnMessages, KACResources.styleSmallButton))
                _ShowAddMessages = !_ShowAddMessages;
            GUILayout.EndHorizontal();
                ...
                && KACWorkerGameState.CurrentVessel != null)
            {
                GUILayout.BeginHorizontal();
                GUILayout.Space(15);
                DrawCheckbox(ref blnAttachVessel, Localizer.Format("#LOC_KAC_85"));
                GUILayout.EndHorizontal();
            }

            //Full width one under the two columns for the kill time warp
            GUILayout.Space(3);
            DrawAlarmActionChoice4(ref Actions, "Action:", 50);

            if (TypeOfAlarm != KACAlarm.AlarmTypeEnum.Raw && TypeOfAlarm != KACAlarm.AlarmTypeEnum.EarthTime && TypeOfAlarm != KACAlarm.AlarmTypeEnum.Crew && TypeOfAlarm != KACAlarm.AlarmTypeEnum.ScienceLab)
                DrawTimeEntry(ref Margin, KACTimeStringArray.TimeEntryPrecisionEnum.Hours, Localizer.Format("#LOC_KAC_86"), 60, 40, 20, KACResources.styleAddHeadingNoWrap);
        ...
        internal void DrawTestSoundButton(AudioClip clip, Int32 Repeats)
        {
            ...
            GUIContent btn = new GUIContent(KACResources.btnPlay, Localizer.Format("#LOC_KAC_87"));
            if (KerbalAlarmClock.audioController.isClipPlaying(clip))
            {
                btn = new GUIContent(KACResources.btnStop, Localizer.Format("#LOC_KAC_88"));
                blnStop = true;
            }
        ...
        internal static String FilterNumericInput(String strInput, Int32 intMaxLength, Boolean blnAllowNegative = true)
        {
            String strFiltered = System.Text.RegularExpressions.Regex.Replace(strInput, blnAllowNegative ? "[^0-9-]" : "[^0-9]", "");
            Boolean blnNegative = strFiltered.StartsWith("-");
            strFiltered = System.Text.RegularExpressions.Regex.Replace(strFiltered, "[^0-9]", "");
            strFiltered = System.Text.RegularExpressions.Regex.Replace(strFiltered, "^0+(?=.)", "");
            if (blnNegative)
                strFiltered = "-" + strFiltered;
            if (strFiltered.Length > intMaxLength)
                strFiltered = strFiltered.Substring(0, intMaxLength);
            return strFiltered;
        }

        internal static String TruncateInput(String strInput, Int32 intMaxLength)
        {
            return strInput.Length > intMaxLength ? strInput.Substring(0, intMaxLength) : strInput;
        }

        internal static Boolean DrawTextField(ref String Value, String RegexValidator, Boolean RegexFailOnMatch, String LabelText = "", Int32 FieldWidth = 0, Int32 LabelWidth = 0, Boolean Locked = false, GUIStyle CaptionStyle = null)
        {
        ...
        {
            Boolean blnReturn = false;
            GUILayout.BeginHorizontal(KACResources.styleGroupTopAligned);
            //GUILayout.Label(LabelText, KACResources.styleAddHeading, GUILayout.Width(LabelWidth - 10));
            GUILayout.Label(LabelText, KACResources.styleAddHeadingNoWrap, GUILayout.Width(LabelWidth - 10));
            ...
            ////blnReturn = DrawRadioList(ref intChoice, "Message", "Kill Warp", "Pause");
            //Choice = (KACAlarm.AlarmActionEnum)intChoice;
            GUIStyle styleButton = KACResources.styleButtonListAlarmActions34;
            ...
            {
                GUI.enabled = false;
                GUILayout.Toggle(false, "", KACResources.styleButtonListBlank);
                GUI.enabled = true;
                GUILayout.Space(-7);
                GUILayout.Label("", KACResources.styleAddHeadingCentered, GUILayout.Width(100));
                rectMsgCap = GUILayoutUtility.GetLastRect();
            ...
            GUILayout.EndHorizontal();
            Rect rectRow = GUILayoutUtility.GetLastRect();
            if (Actions.Warp == AlarmActions.WarpEnum.PauseGame && rectRow.height > 0 && rectMsgCap.width > 0)
            {
                String strNote = Localizer.Format("#LOC_KAC_99");
                GUI.Label(new Rect(rectMsgCap.x, rectRow.y - 4, 100, KACResources.styleAddXferNameUpper.CalcHeight(new GUIContent(strNote), 100)), strNote, KACResources.styleAddXferNameUpper);
            }
            return blnReturn;
        }
        ...
        internal Boolean DrawTimeEntry(ref KACTimeStringArray time, KACTimeStringArray.TimeEntryPrecisionEnum Prec, String LabelText, Int32 LabelWidth, Int32 FieldWidth, Int32 SuffixWidth, GUIStyle HeadingStyle = null, params GUILayoutOption[] options)
        {
            Boolean blnReturn = false;

            GUILayout.BeginHorizontal();
            if (LabelText != "")
                GUILayout.Label(LabelText, HeadingStyle ?? KACResources.styleAddHeadingNoWrap, GUILayout.Width(LabelWidth));

            String strTemp;
            if (Prec >= KACTimeStringArray.TimeEntryPrecisionEnum.Years)
            {
                strTemp = time.Years;
                if (DrawTimeField(ref strTemp, Localizer.Format("#LOC_KAC_100"), FieldWidth, SuffixWidth))
                {
                    blnReturn = true;
                    time.Years = strTemp;
                }
            ...
            {
                strTemp = time.Hours;
                if (DrawTimeField(ref strTemp, Localizer.Format("#LOC_KAC_101"), FieldWidth, SuffixWidth))
                {
                    blnReturn = true;
                    time.Hours = strTemp;
                }
            ...
            if (Prec >= KACTimeStringArray.TimeEntryPrecisionEnum.Minutes)
            {
                ...
                if (DrawTimeField(ref strTemp, Localizer.Format("#LOC_KAC_102"), FieldWidth, SuffixWidth))
                {
                    blnReturn = true;
            ...
            if (Prec >= KACTimeStringArray.TimeEntryPrecisionEnum.Seconds)
            {
                ...
                if (DrawTimeField(ref strTemp, Localizer.Format("#LOC_KAC_103"), FieldWidth, SuffixWidth))
                {
                    blnReturn = true;
        ...
        internal Boolean DrawTimeField(ref String Value, String LabelText, Int32 FieldWidth, Int32 SuffixWidth)
        {
            ...
            if (blnReturn)
            {
                String strFiltered = FilterNumericInput(Value, 3);
